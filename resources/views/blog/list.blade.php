            ...
            <div class="actions">
                ...
                <div class="button-container">
                    <a href="#" class="filter-link">
                        ...
                        <a href="{{ route('blog.create') }}" class="add-patient-btn">{{ __('site.Add New Blog') }}</a>
                </div>
            </div>
                    ...
                    <tbody id="patientTbody">
                        @foreach($blogs as $blog)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('blog.show', $blog) }}" class="btn view-btn">{{ __('site.View') }}</a>
                                    <a href="{{ route('blog.edit', $blog) }}" class="btn edit-btn">{{ __('site.Edit') }}</a>

                                    <form class="delete-form" action="{{ route('blog.destroy', $blog) }}" method="post" class="m-0"
                                        id="deleteForm-{{ $blog->id }}">
                                        @csrf
                                        @method('delete')
                                        <a class="btn delete-btn"
                                            onclick="event.preventDefault(); document.getElementById('deleteForm-{{ $blog->id }}').submit();">
                                            <strong>X</strong>
                                        </a>
                                    </form>
                                </td>
